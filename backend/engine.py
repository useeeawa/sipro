...
def start_scheduler():
    ...
    # ambang anggaran 01:00 UTC (08:00 WIB) supaya tugas FN-11 ada sebelum jam kerja.
    from scheduler_p45 import budget_alert_tick, targets_recalc_tick
    _scheduler.add_job(targets_recalc_tick, "interval", seconds=21600, id="targets_recalc",
                       max_instances=1, coalesce=True)
    _scheduler.add_job(budget_alert_tick, "cron", hour=1, minute=0, id="budget_alert",
                       max_instances=1, coalesce=True)
    _scheduler.start()
